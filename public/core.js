        ...
        async function cloudSave(key, value) {
            ...
            try {
                ...
                updateSyncStatus('?†Ô? Sync failed', 'var(--warn)');
            }
        }
        ...
        async function deleteCloudProfile() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    showToast('??Ô∏?Cloud backup deleted ??now running locally');
                } else {
                    showToast('?†Ô? Delete failed. Try again.');
                }
            } catch(e) {
                showToast('?†Ô? No connection. Try again.');
            }
        }
        ...
        async function chooseStorageMode(mode) {
            if (mode === 'local') {
                ...
                renderStorageModeUI();
                showToast('?ì± Saved to this device only');
                initApp();
                showOnboarding();
                return;
            }
        ...
        function resetPinUI(step) {
            ...
            document.getElementById('pinIcon').innerText    = isRestore ? '??' : '?ÅÔ?';
        ...
        async function submitPinEntry() {
            ...
            if (entered.length < 4) { msg.innerText = '?†Ô? Enter at least 4 digits'; msg.style.color = 'var(--warn)'; return; }
            ...
            if (window._pinStep === 'create') {
                ...
                if (entered === window._pinFirst) {
                    ...
                    if (inUse) {
                        ...
                        warningDiv.innerHTML = `
                            <div style="color:#ff9f0a;font-weight:800;margin-bottom:6px;font-size:13px;">?†Ô? Code Already In Use</div>
                ...
                try {
                    ...
                    msg.innerText = '?†Ô? No connection. Check internet.'; msg.style.color = 'var(--warn)';
        ...
        function dismissCodeWarning() {
            ...
            resetPinUI('create');
            const msg = document.getElementById('pinMsg');
            msg.innerText = '?†Ô? Choose a different code'; msg.style.color = 'var(--warn)';
        ...
        async function restoreExistingInstead(code) {
            document.getElementById('codeWarning')?.remove();
            const msg = document.getElementById('pinMsg');
            msg.innerText = '?ÅÔ? Restoring your backup...'; msg.style.color = 'var(--accent)';
        ...
        async function finalizePinSetup(pin) {
            ...
            document.getElementById('storageSetup').style.display = 'none';
            if (restored === true) {
                showToast('?ÅÔ? Backup restored successfully!');
                sessionStorage.setItem('_efb_just_restored', '1');
            } else if (restored === 'empty') {
                showToast('?ÅÔ? Cloud linked ??no prior data found');
            } else {
        ...
        async function fetchApiStats() {
            ...
            try {
                ...
                content.innerHTML = `<div style="text-align:center;padding:40px;color:var(--danger);"><div style="font-size:32px;margin-bottom:12px;">?†Ô?</div><div>Error: ${err.message}</div></div>`;
            }
        }
            ...
            let html = `
                ...
                <div style="background:#1c1c1e;border:1px solid #333;border-radius:10px;padding:14px;margin-bottom:16px;">
                    ...
                    <div style="display:flex;gap:8px;align-items:center;">
                        <button onclick="generateRandomCode()" style="background:#2c2c2e;border:1px solid #444;color:#8e8e93;padding:8px 12px;border-radius:8px;font-size:11px;cursor:pointer;white-space:nowrap;">?é≤ Random</button>
                        ...
                        <div style="padding:16px 18px;">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;">
                                ...
                                <button onclick="inAppCopyMsg()" id="inAppCopyMsgBtn"
                                        style="background:#2c2c2e;border:1px solid #444;color:#fff;padding:10px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">?í¨ Copy Message</button>
                            </div>
        ...
        function openInAppUserDrawer(code) {
            ...
            document.getElementById('inAppCopyMsgBtn').textContent  = '?í¨ Copy Message';
        ...
        async function saveInAppName() {
            ...
            if (!name) { msg.style.color = 'var(--warn)'; msg.textContent = '?†Ô? Name cannot be empty'; return; }
            ...
            } catch(e) { msg.style.color = 'var(--warn)'; msg.textContent = '?†Ô? Network error'; }
        ...
        async function createUser(password) {
            ...
            if (!name) { msg.innerText = '?†Ô? Enter a name'; msg.style.color = 'var(--warn)'; return; }
            if (!code || code.length < 4) { msg.innerText = '?†Ô? Code must be 4+ characters'; msg.style.color = 'var(--warn)'; return; }
            ...
            try {
                ...
                msg.innerText = '?†Ô? Network error'; msg.style.color = 'var(--warn)';
            }
        ...
        function renderApiStats(data) {
            ...
            html += `<div style="margin-top:24px;padding:12px;background:rgba(255,255,255,0.03);border-radius:8px;font-size:11px;color:#555;line-height:1.6;">?πÔ? Stats reset daily at midnight UTC. Keys rotate automatically based on usage.</div>`;
        ...
        async function updateApiQuickStats() {
            ...
            try {
                ...
                if (data.keys) {
                    ...
                    if (exhausted) {
                        label.innerText = '?†Ô? Keys exhausted';
                    ...
                    label.innerText = '?îß Maintenance';
                ...
                label.innerText = '?†Ô? API unreachable';
                label.style.color = 'var(--warn)';
            }
        }
        ...
        function formatVisDisplay(smValue) {
            ...
            // Round to nearest quarter-mile for clean display (e.g. 1.75 ??1¬æ SM)
        ...
        async function initApp() {
            ...
            // ?Ä?Ä Apply correct starting tab BEFORE loading data ?Ä?Ä
        ...
        async function submitAccessCode() {
            ...
            if (code.length < 4) {
                msg.innerText   = '?†Ô? Access code must be at least 4 characters';
            ...
            try {
                ...
                msg.innerText   = '?†Ô? Could not connect. Check your internet.';
                msg.style.color = 'var(--warn)';
                if (btn) { btn.disabled = false; btn.innerText = 'Enter ??; }
            }
        }
        ...
        async function saveDefaultStation() {
            ...
            } else { msg.innerText = "?†Ô? Invalid ICAO"; msg.style.color = "var(--warn)"; }
        ...
        async function resetApp() {
            ...
            showToast('??Ô∏?App reset ??returning to setup...');
        ...
        function locateUser() {
            ...
            if (!navigator.geolocation) {
                showToast('?†Ô? Geolocation not supported on this device');
                btn.innerHTML = '??';
                return;
            }
            ...
            navigator.geolocation.getCurrentPosition(async (pos) => {
                ...
                try {
                    ...
                    if (!data || data.length === 0) {
                        showToast('?†Ô? No aviation weather stations found within 150nm');
                        return;
                    }
...
// ================================================================
        ...
        async function loadData(force = false) {
            ...
            try {
                // ?Ä?Ä PHASE 1: Station data (fastest, ~100ms) ?Ä?Ä
                ...
                if (!stationData || stationData.error) {
                    ...
                    document.getElementById('freqContainer').innerHTML =
                        `<div style="grid-column:1/-1;color:var(--warn);font-size:12px;padding:8px;">
                            ?†Ô? Station data unavailable (${errMsg}).
                ...
                // ?Ä?Ä PHASE 2: Start slow tasks in background (don't block) ?Ä?Ä
                ...
                // ?Ä?Ä PHASE 3: Weather data in parallel - render each as it completes ?Ä?Ä
                ...
                weatherPromises.then(([metarRes, tafRes, notamRes, sigairmetRes]) => {
                    ...
                    if (metarRes.status === 'fulfilled' && !metarRes.value.error) {
                        ...
                        // ?Ä?Ä Sync dashboard card if this airport is tracked ?Ä?Ä
                    ...
                    mirrorToWeatherTab();
                        ...
                        showToast('?†Ô? Data loaded but render failed ??check console');
                    }
                });;
        ...
        function renderNotams(rawData) {
            ...
            const html = renderGroup(critical, '??CRITICAL', '#ff453a')
                       + renderGroup(warning,  '?†Ô? CAUTION',  '#ff9f0a')
                       + renderGroup(info,     '?πÔ? INFO',      '#8e8e93');
        ...
        function renderSigairmet(items, containerId) {
            ...
            container.innerHTML =
                renderGroup(sigmets, '?î¥ SIGMET',        '#ff453a', 'rgba(255,69,58,0.06)')  +
                renderGroup(sierras, '?ü° AIRMET SIERRA', '#ff9f0a', 'rgba(255,159,10,0.06)') +
                renderGroup(tangos,  '?ü¢ AIRMET TANGO',  '#30d158', 'rgba(48,209,88,0.06)')  +
                renderGroup(zulus,   '?îµ AIRMET ZULU',   '#0a84ff', 'rgba(10,132,255,0.06)') +
                renderGroup(other,   '?πÔ? OTHER',          '#8e8e93', 'rgba(255,255,255,0.03)');
        }
        ...
        function updateAudioSection(icao) {
            ...
            if (liveAtcBtn) {
                ...
                liveAtcBtn.querySelector('span').innerText = `?ì° LiveATC (${icao})`;
            }
            ...
            if (metarTafBtn) {
                ...
                metarTafBtn.querySelector('span').innerText = `?ÅÔ? Metar-Taf.com (${icao})`;
            }
                ...
                container.innerHTML = cfg.streams.map(s => `
                    <div style="margin-bottom:12px;">
                        ...
                        <div style="display:none;font-size:11px;color:var(--warn);
                            ...
                            <span>?†Ô? Feed currently offline</span>
        ...
        async function fetchMeteogram(lat, lon) {
            ...
            } catch(e) { console.error("Meteo Error:", e); loader.style.display = 'block'; loader.innerText = "?†Ô? Model Data Unavailable"; }
        }
        ...
        function drawMeteogram(h, canvasId = 'meteoCanvas') {
            ...
            const getWxIcon = (code) => {
                if (code <= 1) return '?ÄÔ∏?; if (code <= 3) return '??; if (code <= 48) return '?å´Ô∏?;
                if (code <= 57) return '?åßÔ∏?; if (code <= 67) return '??; if (code <= 77) return '?ÑÔ?';
                if (code <= 82) return '?àÔ?'; if (code <= 99) return '??; return '?ÅÔ?';
            };
            // Vertical grid
        ...
        // ================================================================
         function renderMetar(d) {
            ...
            if (minAgo > 60) rawContainer.insertAdjacentHTML('beforeend', `<div style="margin-top:12px;padding:10px;background:rgba(255,69,58,0.1);border:1px solid var(--danger);border-radius:6px;text-align:center;"><div style="color:var(--danger);font-weight:800;font-size:12px;">?†Ô? DATA OUTDATED (${minAgo}m old)</div></div>`);
            ...
            // ?Ä?Ä GET CURRENT ICAO FOR TREND ANALYSIS ?Ä?Ä
            const icao = document.getElementById('icao').value.toUpperCase();
            
            // ?Ä?Ä WIND with trend ?Ä?Ä
            ...
            // ?Ä?Ä PRESENT WEATHER (wx_codes) ?Ä?Ä
        ...
        function getBearing(lat1, lon1, lat2, lon2) {
            ...
            const y = Math.sin(?Œª) * Math.cos(?2);
            const x = Math.cos(?1) * Math.sin(?2) -
                      Math.sin(?1) * Math.cos(?2) * Math.cos(?Œª);
        ...
        async function findNearbyStations(lat, lon) {
            ...
            try {
                ...
                console.log(`[Nearby] ?ì° Fetching: ${url}`);
        
                const data = await secureFetch(url);
                console.log('[Nearby] ?ì¶ Raw API response:', data);
                ...
                console.log(`[Nearby] ?ÇÔ?  After filter: ${others.length} stations`);
                ...
                others.forEach(item => {
                    ...
                    // ?Ä?Ä Primary list (METAR tab) ?Ä?Ä
                    ...
                    // ?Ä?Ä Mirror list (Weather tab) ??clone with LIVE onclick handler ?Ä?Ä
                    if (list2) {
        ...
        function renderInfoFrequencies(d, fContainer) {
            ...
            // ?Ä?Ä Layer 1: embedded FREQ_DB ?Ä?Ä
            let dbFreqs = null;
            try {
            ...
            if (dbFreqs && dbFreqs.length > 0) {
                ...
                // ?Ä?Ä Layer 2: AVWX frequencies[] ?Ä?Ä
                ...
                if (avwxFreqs.length > 0) {
                    ...
                    // ?Ä?Ä Layer 3: aviationweather.gov ?Ä?Ä
        ...
        function checkDAWarning(da, elev) {
            ...
            if (da > elev + 2000) { daEl.style.color = "var(--warn)"; daEl.innerHTML = `${da} ft ?†Ô?`; }
            else { daEl.style.color = "var(--text)"; }
        }

        // ?Ä?Ä Opens formula-modal with dynamic title + body ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
